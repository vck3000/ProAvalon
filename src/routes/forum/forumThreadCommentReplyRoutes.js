...
async function createCommentReply(req, res) {
  ...
  if (req.params.replyId) {
    forumThreadCommentReply
      .findById(mongoose.Types.ObjectId(req.params.replyId))
      .exec((err, foundReply) => {
...
async function createReply(req, res, commentReplyData, replyingToThisReply) {
  ...
  const foundForumThreadComment = await forumThreadComment
...
router.get(
  '/:id/:comment_id/:reply_id/edit',
  checkForumThreadCommentReplyOwnership,
  asyncMiddleware(async (req, res) => {
    const foundReply = await forumThreadCommentReply
      .findById(req.params.reply_id)
      .exec();
