...
router.post(
  '/login',
  sanitiseUsername,
  /* usernameToLowerCase, */ passport.authenticate('local', {
      ...
      if (err) {
        ...
        for (let i = 0; i < currentModActions.length; i++) {
          if (
            ...
            currentModActions[i].bannedPlayer.id.toString()
          ) {
            if (currentModActions[i].type === 'ban') {
              ...
              console.log(
                `${req.user.username} is still banned and cannot join the lobby.`
              );
              return;
            }
          }
