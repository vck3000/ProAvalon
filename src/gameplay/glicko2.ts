...
const computeE = (mu, mu_j, phi_j) => {
  return 1 / (1 + Math.exp(-1 * computeG(phi_j) * (mu - mu_j)));
};


const computeNewVolatility = (delta, phi, v, sigma, tau) => {
  const epsilon = 0.000001;
  const a = Math.log(sigma ** 2);
  const computeF = (x) => {
    let output = 0;

    output += Math.E ** x * (delta ** 2 - phi ** 2 - v - Math.E ** x) / (2 * (phi ** 2 + v + Math.E ** x) ** 2);
    output -= (x - a) / tau ** 2;
    return output;
  };
...
export const glicko2Update = (playerData, matchHistory) => {
  ...
  const newVolatility = computeNewVolatility(delta, player.phi, v, player.ratingVolatility, tau);
