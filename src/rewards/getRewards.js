...
async function userHasReward(user, reward, patreonDetails) {
  ...
  // Check for admin
  if (
    reward.adminReq === true &&
    !adminsArray.includes(user.username.toLowerCase())
  ) {
    // Fail case.
    return false;
  }
  ...
  if (
    reward.modReq === true &&
    !modsArray.includes(user.username.toLowerCase())
  ) {
    // Fail case.
    return false;
  }
  ...
  if (
    reward.gamesPlayedReq !== 0 &&
    user.totalGamesPlayed < reward.gamesPlayedReq
  ) {
    // Fail case.
    return false;
  }
