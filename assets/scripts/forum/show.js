  ...
  reply.addEventListener('click', function () {
    ...
    for (let i = 0; i < childNodes.length; i++) {
      if (
        ...
        childNodes[i].classList.contains('replyBox')
      ) {
...
likes.forEach((like) => {
  like.addEventListener('click', function () {
    ...
    xmlhttp.open(
      'GET',
      `/forum/ajax/like/${typeofelement}/${idOfForum}=${idOfComment}=${idOfReply}`,
      true
    );
    ...
    xmlhttp.onreadystatechange = function () {
      if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
        ...
        if (message === 'liked') {
          $(thisLike).notify('Liked', {
            position: 'right',
            autoHideDelay: 1000,
            className: 'success',
          });
          $(`#${idofelement}likes`)[0].innerText =
            parseInt($(`#${idofelement}likes`)[0].innerText) + 1;
          $(thisLike)[0].innerText = 'Unlike';
        } else if (message === 'unliked') {
          $(thisLike).notify('Unliked', {
            position: 'right',
            autoHideDelay: 1000,
            className: 'info',
          });
          ...
          $(thisLike).notify('You need 10 games to like a forum/comment.', {
            position: 'right',
            autoHideDelay: 3000,
            className: 'error',
          });
        } else {
          $(thisLike).notify('Error! Something went wrong...', {
            position: 'right',
            autoHideDelay: 1000,
            className: 'error',
          });
