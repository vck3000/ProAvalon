...
function drawData() {
    ...
    let ctx = document.getElementById("gamesPlayedChart");
    let myChart = new Chart(ctx, gamesPlayedStatsConfig);

    //***********************************
    //General stats
    //***********************************
    let resWinPercent = Math.round(grd.totalResWins/grd.totalgamesplayed*100);
    let spyWinPercent = Math.round(grd.totalSpyWins/grd.totalgamesplayed*100);
    ...
    //Assassination stats
    //***********************************
    let totalAssassinShots = grd.assassinRolesShot["Merlin"] + 
    ...
    for (let i = 5; i <= 10; i++)
        $(`#${i}paveragegameduration`)[0].innerText = new Date(grd[`${i}paverageGameDuration`]).getUTCMinutes() + " mins " + new Date(grd[`${i}paverageGameDuration`]).getUTCSeconds() + " seconds";
