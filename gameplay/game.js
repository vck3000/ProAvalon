...
Game.prototype.checkBotMoves = function (pendingBots) {
    ...
    this.interval = setInterval(() => {
        ...
        thisRoom.botSockets.forEach((botSocket) => {
            ...
            var availablePlayers = thisRoom.playerUsernamesInGame.filter((_, playerIndex) => prohibitedIndexesToPick.indexOf(playerIndex) === -1);
...
Game.prototype.getRoomPlayers = function () {
	if (this.gameStarted === true) {
		...
		for (let i = 0; i < this.playersInGame.length; i++) {
			...
			const isClaiming = this.claimingPlayers.indexOf(this.playersInGame[i].request.user.username) !== -1;
