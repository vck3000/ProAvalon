...
function Game (host_, roomId_, io_, maxNumPlayers_, newRoomPassword_){
	...
	// Frozen games. When do we delete them? When the cutoff number of players have joined
	// Or when the room has been there for more than 5 mins and someone leaves it (this part is in sockets.js)
	this.someCutoffPlayersJoined = "no";
	this.cutoffForSomePlayersJoined = 5;

	// Chat history is in this game.js file because we only start recording
	// chat after the game has started
    this.chatHistory = [];
	...
	//Player standing up, only let them stand up under these conditions
	...
	//If there is no one in the room, destroy
